...
function job()
{
	...
	source build.sh "$THIS_DIR" $configuration  # Source to apply environment variables in the case
	[ $? -ne 0 ] && log_error " --- Errors during building" && exit 1
	source $THIS_DIR/deploy.sh $@  # Source to apply environment variables in the case
